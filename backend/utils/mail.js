...
const WarningMail = (
  ...
  transport
    .sendMail({
      ...
      to: recieverEmail,
      subject: `Alert`,
      html: `Hello ${name} <br> Your attendance percentage in ${monthname} was lesser than 50%. <br> Kindly start attending the classes.`,
    })
    .catch((err) => console.log(err));
};
const forgetpasswordmail = (
  ...
  transport
    .sendMail({
      ...
      to: email,
      subject: `Password`,
      html: `Hi ${name}, '${password}' is your login password for Classopedia`
    })
    .catch((err) => console.log(err));
};

module.exports = { classScheduleMail, testScheduleMail,AssignmentMail,StudyMaterialMail,MonthlyMail,WarningMail,forgetpasswordmail };
